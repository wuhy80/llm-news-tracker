...
文章全文翻译由独立的 `translate-articles.yml` 每 5 分钟增量运行，默认优先使用 `z-ai/glm-5.2:free`，每轮最多请求 48 次，并按 UTC 日期限制每天最多 1,000 次。可通过 Repository variable `ARTICLE_TRANSLATION_MODEL` 更换模型或改回 `openrouter/free`；全文翻译使用较小分块，以减少免费模型返回英文、遗漏段落或生成不完整 JSON 的概率。
